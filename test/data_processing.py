...
agg_rules = {
    ...
    # 状态量/通量用 mean (算出年平均水平)
    # 注意：辐射J/m2在这里取mean代表"年平均月累积量"，
    # 如果想得到严格的"年总辐射能"，可以改为sum，但做相关性分析时mean更常用
...
def process_data():
    ...
    for i, (_, row) in enumerate(unique_sites.iterrows()):
        ...
        try:
            # A. 空间提取 (Nearest)
            ds_point = ds.sel(latitude=lat, longitude=q_lon, method='nearest')
            
            # B. 时间重采样 (Month -> Year)
            # 使用 '1YE' (Year End) 或 '1Y'
            ds_yearly = ds_point.resample({time_dim: '1YE'})
            
            df_site = pd.DataFrame()
            # 获取年份
            df_site['YEAR'] = ds_yearly.mean()[time_dim].dt.year
            ...
            for nc_var, csv_col in var_map.items():
                ...
                if rule == 'sum':
                    vals = ds_yearly.sum()[nc_var].values
                else:
                    vals = ds_yearly.mean()[nc_var].values
                
                df_site[csv_col] = vals
