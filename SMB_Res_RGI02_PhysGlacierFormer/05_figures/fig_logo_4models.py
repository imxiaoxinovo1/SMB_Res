"""Four-panel LOGO scatter plot for RF, XGBoost, LSTM, and GlacierFormer."""
...
MODELS = [
    ("RF", os.path.join(LEGACY_RESULTS, "rf_logo_predictions.csv")),
    ("XGBoost", os.path.join(LEGACY_RESULTS, "xgboost_logo_predictions.csv")),
    ("LSTM", os.path.join(LEGACY_RESULTS, "lstm_logo_predictions.csv")),
    (
        "GlacierFormer",
        os.path.join(
            CURRENT_RESULTS,
...
OUT_PNG = os.path.join(FIG_DIR, "fig_logo_4models.png")
...
def main() -> None:
    ...
    for ax, (label, obs, pred) in zip(axes, loaded):
        ...
        colorbar = fig.colorbar(
            ScalarMappable(norm=Normalize(0, density.max()), cmap="plasma"),
            ax=ax,
            fraction=0.046,
            pad=0.025,
        )
    ...
    plt.close(fig)
    print(f"Saved LOGO figure -> {OUT_PNG}")
