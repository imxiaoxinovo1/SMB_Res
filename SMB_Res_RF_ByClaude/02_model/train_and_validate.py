...
- Leave-One-Glacier-Out (LOGO): 评估空间泛化能力 (新增)
- 特征重要性分析
"""
import sys
import os
...
from config import (TRAINING_DATA_CSV, MODEL_RESULTS_DIR, FEATURES, TARGET, RF_PARAMS,
...
# sklearn RF (>=1.4) 支持特征列中的 NaN，只需保证 TARGET 无 NaN
df = df.dropna(subset=[TARGET])
print(f"   清洗后行数: {len(df)}")

# 年份过滤 (与 test_rf_fixed.py 保持一致)
...
# ================= 3. LOYO 验证 =================
print("\n" + "=" * 60)
print(">>> 3. Leave-One-Year-Out 交叉验证 (LOYO)")
print("=" * 60)
...
# ================= 4. LOGO 验证 =================
print("\n" + "=" * 60)
print(">>> 4. Leave-One-Glacier-Out 交叉验证 (LOGO)")
print("=" * 60)
...
# ================= 6. 保存结果 =================
print("\n>>> 6. 保存结果...")
df_loyo_summary.to_csv(os.path.join(MODEL_RESULTS_DIR, "loyo_summary.csv"), index=False)
df_loyo_preds.to_csv(os.path.join(MODEL_RESULTS_DIR, "loyo_predictions.csv"), index=False)
df_logo_summary.to_csv(os.path.join(MODEL_RESULTS_DIR, "logo_summary.csv"), index=False)
df_logo_preds.to_csv(os.path.join(MODEL_RESULTS_DIR, "logo_predictions.csv"), index=False)
pd.DataFrame(feat_imp_data).to_csv(os.path.join(MODEL_RESULTS_DIR, "feature_importance.csv"), index=False)
