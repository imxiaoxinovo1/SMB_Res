# 02_models/rf/train_logo.py
"""Random Forest LOGO (Leave-One-Glacier-Out) cross-validation."""
import sys, os
...
pd.DataFrame(fold_r2).to_csv(os.path.join(RESULT_DIR, 'rf_logo_metrics.csv'), index=False)
pd.DataFrame({'obs': obs, 'pred': preds}).to_csv(
    os.path.join(RESULT_DIR, 'rf_logo_predictions.csv'), index=False)
pd.DataFrame([{'model': 'rf', 'cv': 'LOGO',
               'r2': r2_global, 'pearson_r': pearson_r,
               'rmse_mm': rmse_global*1000, 'bias_mm': bias*1000}]).to_csv(
    os.path.join(RESULT_DIR, 'rf_logo_summary.csv'), index=False)
print(f"Saved → {RESULT_DIR}/rf_logo_*.csv")
