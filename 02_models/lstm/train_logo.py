# 02_models/lstm/train_logo.py
"""Simple LSTM LOGO (Leave-One-Glacier-Out) cross-validation."""
import sys, os
...
log_path = os.path.join(RESULT_DIR, 'lstm_logo_train.log')
log = open(log_path, 'w', encoding='utf-8', buffering=1)

def log_print(msg):
    print(msg); log.write(msg + '\n'); log.flush()

log_print("=== Simple LSTM LOGO ===")
...
pd.DataFrame({'obs': all_obs, 'pred': all_preds}).to_csv(
    os.path.join(RESULT_DIR, 'lstm_logo_predictions.csv'), index=False)
pd.DataFrame([{'model': 'lstm', 'cv': 'LOGO',
               'r2': r2, 'pearson_r': pearson_r,
               'rmse_mm': rmse*1000, 'bias_mm': bias*1000}]).to_csv(
    os.path.join(RESULT_DIR, 'lstm_logo_summary.csv'), index=False)
