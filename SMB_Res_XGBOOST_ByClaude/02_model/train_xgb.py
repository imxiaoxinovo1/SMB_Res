...
from config import (TRAINING_DATA_CSV, MODEL_RESULTS_DIR, FEATURES, TARGET, XGB_PARAMS,
...
# XGBoost 原生支持特征 NaN，只需保证 TARGET 无 NaN
...
# ================= 3. LOYO 验证 =================
print("\n" + "=" * 60)
print(">>> 3. Leave-One-Year-Out 交叉验证 (LOYO) - XGBoost")
print("=" * 60)
...
# ================= 4. LOGO 验证 =================
print("\n" + "=" * 60)
print(">>> 4. Leave-One-Glacier-Out 交叉验证 (LOGO) - XGBoost")
print("=" * 60)
...
# ================= 6. 保存结果 =================
print("\n>>> 6. 保存结果...")
df_loyo_summary.to_csv(os.path.join(MODEL_RESULTS_DIR, "xgb_loyo_summary.csv"), index=False)
df_loyo_preds.to_csv(os.path.join(MODEL_RESULTS_DIR, "xgb_loyo_predictions.csv"), index=False)
df_logo_summary.to_csv(os.path.join(MODEL_RESULTS_DIR, "xgb_logo_summary.csv"), index=False)
df_logo_preds.to_csv(os.path.join(MODEL_RESULTS_DIR, "xgb_logo_predictions.csv"), index=False)
pd.DataFrame(feat_imp_data).to_csv(os.path.join(MODEL_RESULTS_DIR, "xgb_feature_importance.csv"), index=False)
