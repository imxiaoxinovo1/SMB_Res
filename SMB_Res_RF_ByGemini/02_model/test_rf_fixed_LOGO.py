...
# ================= 3. LOGO-CV 循环 (空间交叉验证) =================
results_spatial = {'wgms_id': [], 'y_test': [], 'y_pred': []}

glacier_ids = df['WGMS_ID'].unique()
print(f"\n>>> 开始按冰川循环执行空间验证 (共 {len(glacier_ids)} 个冰川)...")

for test_id in glacier_ids:
    # 训练集: 不包含 test_id 冰川的所有数据 (含分带)
    train_data = df[df['WGMS_ID'] != test_id]
    # 测试集: 仅包含 test_id 冰川 且 TAG=9999 (全冰川数据)
    test_data = df[(df['WGMS_ID'] == test_id) & (df['TAG'] == 9999)]
    ...
    if len(X_test) > 0:
        ...
        results_spatial['wgms_id'].extend([test_id] * len(y_test_m))
        results_spatial['y_test'].extend(y_test_m)
        results_spatial['y_pred'].extend(y_pred_m)

# ================= 4. 最终评估 =================
# 计算整体指标
y_true_all = np.array(results_spatial['y_test'])
y_pred_all = np.array(results_spatial['y_pred'])
...
plt.title("Feature Importance (Trained on All Bands)")
plt.xlabel("Importance Score")
plt.tight_layout()
plt.show()

# ================= 6. 绘制论文同款核密度散点图 =================
print("\n>>> 正在绘制空间交叉验证 (LOGO-CV) 的密度散点图...")
# 计算点密度
...
ax.set_title('LOGO-CV (Spatial): Predicted vs Ground Truth', fontsize=16)
